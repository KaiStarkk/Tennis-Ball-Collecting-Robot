...
while display.isNotDone():
    ...
    dist = img.hueDistance(hue, insat, inval)
    ...
    for c in circles:
        img.drawCircle((c.x, c.y), c.radius(), impleCV.Color.RED, 3)
        img.drawText(str(len(circles) - count),
                     c.x, c.y,
                     impleCV.Color.BLUE,
                     nt(c.radius()) - 10)
        count += 1
    ...
    panel.polygon(((0, 0), (0, 60),
                   (200, 60), (200, 0)
                   ),
                  (100, 100, 100), 0, True, True, 200)
